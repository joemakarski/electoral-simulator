'use client';

import { useSimulationStore } from '@/store/simulationStore';

// Helper to map party IDs to pleasing UI colors. Parties hardcoded for now.
const PARTY_COLORS: Record<string, string> = {
  "RED": "bg-red-500",
  "BLUE": "bg-blue-500",
  "GREEN": "bg-green-500",
  "DEFAULT": "bg-gray-500"
};

export default function ResultsDashboard() {
  const { results, candidates, districts } = useSimulationStore();

  // If there are no results yet, don't render anything
  if (!results) return null;

  // Data lookups as objects
  const partyLookup = Object.fromEntries(candidates.map(c => [c.id, c.party_id]));
  const candidateLookup = Object.fromEntries(candidates.map(c => [c.id, c.name]));
  const districtLookup = Object.fromEntries(districts.map(d => [d.id, d.name]));
  districtLookup["NATIONAL_LIST"] = "National Top-Up List";

  // Tally seats by party
  const seatsByParty: Record<string, number> = {};
  Object.entries(results.winners).forEach(([_, winnersList]) => { // e.g. ["d1", ["c1", "c2"]]
    (winnersList as string[]).forEach((wId) => { // lookup their party and increment accordingly
      const pId = partyLookup[wId] || "UNKNOWN";
      seatsByParty[pId] = (seatsByParty[pId] || 0) + 1;
    });
  });

  // Calculate totals for percentages
  const totalSeats = results.stats.total_parliament_size;
  const nationalVotes: Record<string, number> = results.results.national_party_votes;
  const totalVotes = Object.values(nationalVotes).reduce((a, b) => a + b, 0);

  // Get a unique list of all parties that either got votes or won seats
  const allParties = Array.from(new Set([...Object.keys(nationalVotes), ...Object.keys(seatsByParty)]));

  return (
    <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200 mt-6 animate-fade-in">
      <h2 className="text-2xl font-bold mb-6 text-gray-800">Election Results</h2>

      {/* Warning Notes Section */}
      {results.stats.notes && results.stats.notes.length > 0 && (
        <div className="mb-8 p-4 bg-amber-50 border-l-4 border-amber-500 rounded-r-md">
          <h3 className="text-amber-800 font-bold mb-2">Simulation Notes:</h3>
          <ul className="list-disc list-inside text-sm text-amber-700 space-y-1">
            {results.stats.notes.map((note: string, idx: number) => (
              <li key={idx}>{note}</li>
            ))}
          </ul>
        </div>
      )}

      {/* Proportionality Comparison Section */}
      <div className="mb-10">
        <h3 className="text-lg font-bold mb-4 border-b pb-2">National Proportionality</h3>
        <div className="flex flex-col gap-6">
          {allParties.map((party) => {
            const votes = nationalVotes[party] || 0;
            const seats = seatsByParty[party] || 0;
            
            const votePct = totalVotes > 0 ? (votes / totalVotes) * 100 : 0;
            const seatPct = totalSeats > 0 ? (seats / totalSeats) * 100 : 0;
            const colorClass = PARTY_COLORS[party] || PARTY_COLORS["DEFAULT"];

            return (
              <div key={party} className="flex flex-col md:flex-row gap-4 md:items-center">
                <div className="w-24 font-bold text-gray-700">{party}</div>
                
                {/* Vote Share Bar */}
                <div className="flex-1">
                  <div className="flex justify-between text-xs text-gray-500 mb-1">
                    <span>Popular Vote</span>
                    <span>{votePct.toFixed(1)}% ({votes.toLocaleString()})</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-3">
                    <div className={`${colorClass} h-3 rounded-full opacity-70`} style={{ width: `${votePct}%` }}></div>
                  </div>
                </div>

                {/* Seat Share Bar */}
                <div className="flex-1">
                  <div className="flex justify-between text-xs text-gray-500 mb-1">
                    <span>Seats Won</span>
                    <span>{seatPct.toFixed(1)}% ({seats} seats)</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-3">
                    <div className={`${colorClass} h-3 rounded-full`} style={{ width: `${seatPct}%` }}></div>
                  </div>
                </div>
              </div>
            );
          })}
        </div>
      </div>

      {/* Local District Winners Section */}
      <div>
        <h3 className="text-lg font-bold mb-4 border-b pb-2">District Breakdown</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Object.entries(results.winners).map(([dId, winnersList]) => {
            const wList = winnersList as string[];
            if (wList.length === 0) return null;

            return (
              <div key={dId} className="p-4 border rounded bg-gray-50 shadow-sm">
                <h4 className="font-bold text-gray-700 mb-2">{districtLookup[dId] || dId}</h4>
                <div className="flex flex-col gap-2">
                  {wList.map((wId) => {
                    const party = partyLookup[wId] || "DEFAULT";
                    const colorClass = PARTY_COLORS[party] || PARTY_COLORS["DEFAULT"];
                    return (
                      <div key={wId} className="flex items-center gap-2 text-sm bg-white p-2 border rounded">
                        <div className={`w-3 h-3 rounded-full ${colorClass}`}></div>
                        <span className="font-semibold">{candidateLookup[wId] || wId}</span>
                        <span className="text-gray-500 text-xs">({party})</span>
                      </div>
                    );
                  })}
                </div>
              </div>
            );
          })}
        </div>
      </div>
    </div>
  );
}